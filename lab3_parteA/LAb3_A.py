...
class LAb3_A(gr.top_block, Qt.QWidget):

    def __init__(self, Ac_1=1, ka=1):
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "LAb3_A")
        ...
        self._k_range = qtgui.Range(0, 1, 1, 1, 200)
        ...
        self._Ac_win = qtgui.RangeWidget(self._Ac_range, self.set_Ac, "amplitud de portadora", "counter_slider", float, QtCore.Qt.Horizontal)
...
def argument_parser():
    ...
    parser.add_argument(
        "--Ac-1", dest="Ac_1", type=eng_float, default=eng_notation.num_to_str(float(1)),
...
def main(top_block_cls=LAb3_A, options=None):
    if options is None:
        options = argument_parser().parse_args()

    qapp = Qt.QApplication(sys.argv)

    tb = top_block_cls(Ac_1=options.Ac_1, ka=options.ka)
